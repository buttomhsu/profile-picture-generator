...
")");1==$userimage.hasClass("dragged")?$userimage.attr("class","inner dragged"):$userimage.attr("class","inner");$("<img/>").attr("src",getBackgroundImage($userimage)).load(function(){var b=[this.width,this.height],e=$userimage.width();resizeDragger(b,e,a)})});$("#normalSubmit").click(function(){var a=$userimage.width(),b=getBackgroundSize($userimage.css("background-size")),e=getBackgroundPosition($userimage.css("background-position")),a=a/960,d=$("input[name=template]:checked").val(),c=$("#source").val();
createImage(d,c,e[0]/a,e[1]/a,b[0]/a,b[1]/a)})});$(window).konami({code:[55,55,55],cheat:function(){$(".banana").slideDown()}});
$(window).konami({code:[54,54,54],cheat:function(){$("h1,#size-slider").delay(100).animate({opacity:"0"},2900);$("#formbuttons,.template-label").delay(400).animate({opacity:"0"},2600);$("#settings").delay(1E3).animate({opacity:"0"},2E3);$(".left-bottom-corner").delay(800).animate({opacity:"0"},2200);$(".preview").animate({top:"-960px",opacity:"0.5"},3E3).animate({width:"0",opacity:"0"},3E3,function(){$("#content").slideUp()})}});
function createImage(a,b,e,d,c,g){var f=new Image;f.src="images/object/"+a+".png";a=new Image;a.src=b;b=document.getElementById("result");b.width=960;b.height=720;var h=b.getContext("2d");h.rect(0,0,960,720);h.fillStyle="#CCCCCC";h.fill();h.drawImage(a,e,d,c,g);h.drawImage(f,0,0,960,720);e=b.toDataURL("image/png");$("#download").attr("href",e);$("#download")[0].click()}
$(function(){var a=document.getElementById("drop");a.addEventListener("dragover",handleDragOver,!1);a.addEventListener("drop",handleFileSelect,!1);$(".uploadBtn").click(function(){$("#uploadInput").click()});$("#uploadInput").on("change",function(){input=document.getElementById("uploadInput");loadImage(input.files)})});function handleFileSelect(a){a.stopPropagation();a.preventDefault();loadImage(a.dataTransfer.files)}
function handleDragOver(a){a.stopPropagation();a.preventDefault();a.dataTransfer.dropEffect="copy"}
...
$("#templates label").css("background-image","url("+b+")");$("<img/>").attr("src",a).load(function(){var a=$("input[name=template]:checked").val(),b=$userimage.width();resizeDragger([this.width,this.height],b,a);$loading.hide();$uploading.fadeOut()})}$uploading.fadeIn();$loading.show();var d,c;a?(a=a[0],d=new FileReader,d.onload=b,d.readAsDataURL(a)):alert("\u60b2\u5287\uff01\u60a8\u7684\u700f\u89bd\u5668\u4e0d\u652f\u63f4\u6a94\u6848\u4e0a\u50b3\uff01")}
